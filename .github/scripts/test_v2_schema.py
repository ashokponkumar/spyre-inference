...
def test_column_order_matches_the_pre_refactor_lists():
    ...
    assert list(TEST_CASES.columns) == ["test_case_id", "component", "classname", "name", "tags"]
...
def test_empty_required_column_is_refused():
    with pytest.raises(SchemaError, match="must be non-empty"):
        TEST_CASES.row(
            {"test_case_id": "u", "component": "", "classname": "k", "name": "n", "tags": []}
        )
...
def test_registry_covers_exactly_the_four_v2_tables():
    assert set(TABLES) == {"test_cases", "test_case_runs", "benchmarks", "benchmark_runs"}
